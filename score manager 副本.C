...
/*D·打开文件*/void name()
{
	...
	scanf("%d",&i);
	if(i==1)
		{printf("请输入一个学生信息文件的文件名：");scanf("%s",ch1);}
	else 
		strcpy(ch1,"Students.txt");
	if((f1=fopen(ch1,"r+"))==NULL) 
		{printf("文件名有误，即将打开默认文件");system("pause");f1=fopen("Students.txt","r+");}
	printf("\n是否指定课程信息txt文件？\n是：1和<Enter>\n否：0和<Enter>.\n");
	scanf("%d",&i);
	if(i==1)   
		{printf("请输入一个课程信息文件的文件名：");scanf("%s",ch2);}
	else 
		strcpy(ch2,"Course.txt");
	if((f2=fopen(ch2,"r+"))==NULL) 
		{printf("文件名有误，即将打开默认文件");system("pause");f2=fopen("Course.txt","r+");}
	printf("\n是否指定选课信息txt文件？\n是：1和<Enter>\n否：0和<Enter>.\n");
	scanf("%d",&i);
	if(i==1)   
		{printf("请输入一个选课信息文件的文件名：");scanf("%s",ch3);}
	else 
		strcpy(ch3,"Selection.txt");
	if((f3=fopen(ch3,"r+"))==NULL) 
		{printf("文件名有误，即将打开默认文件");system("pause");f3=fopen("Selection.txt","r+");}
	init();
	ml();
...
/*D·初始化数据,加载至结构体*/void init()
{
	int i=0;
	while(feof(f1)==0)
		{fscanf(f1,"%s%s%s%d",&st[i].xh,&st[i].xm,&st[i].xb,&st[i].nl);i++;}
	num_s=i;
	i=0;
	while(feof(f2)==0)
		{fscanf(f2,"%s%s%f",&c[i].kh,&c[i].km,&c[i].xf);i++;}
	num_c=i;
	i=0;
	while(feof(f3)==0)
		{fscanf(f3,"%s%s%f",&sc[i].xh,&sc[i].kh,&sc[i].cj);i++;}
	num_sc=i;
	fclose(f1);
	fclose(f2);
	fclose(f3);
...
/*D·make成绩单*/void creat_cjlist()
{
	...
	for(i=0;i<num_s;i++)
	{
		...
		{
			if(CJ[i].cj[m]<0);
			else CJ[i].pj=CJ[i].pj+CJ[i].cj[m]/(num_c-n);
		}
	}
}
...
/*D·成绩·平均降序*/void sort_cjlist()
{
	...
	{	
		...
		for(j=k=i;j<num_s;j++)
			...
			strcpy(xh,CJ[i].xh);strcpy(CJ[i].xh,CJ[k].xh);strcpy(CJ[k].xh,xh);
			strcpy(xm,CJ[i].xm);strcpy(CJ[i].xm,CJ[k].xm);strcpy(CJ[k].xm,xm);
		for(m=0;m<num_c;m++)
		{
			cj=CJ[i].cj[m];CJ[i].cj[m]=CJ[k].cj[m];CJ[k].cj[m]=cj;
		}
		pj=CJ[i].pj;CJ[i].pj=CJ[k].pj;CJ[k].pj=pj;
		zxf=CJ[i].zxf;CJ[i].zxf=CJ[k].zxf;CJ[k].zxf=zxf;	
	}	
}
...
re:
			...
			scanf("%d",&t);
			if(t==1) th(i);
			else if(t==2) {printf("将添加信息");system("pause");strcpy(st[num_s].xh,xh);zj();num_s++;}
			else if(t==0) goto end;
			else {system("cls");goto re;}
		}
	}	
	if(p!=1)
		{  strcpy(st[num_s].xh,xh);zj();num_s++;}
...
re:
			...
			scanf("%d",&t);
			if(t==1) gx(i);
			else if(t==2) {printf("将增加至信息");system("pause");strcpy(c[num_c].kh,kh);tj();num_c++;}
			else if(t==0) goto end;
			else {system("cls");goto re;}
		}
	}	
	if(p!=1) {  strcpy(c[num_c].kh,kh);tj();num_c++;}
...
head:
	...
	if(p!=1)
	{
		...
		if(p!=1)
		{
			...
			scanf("%d",&j);
			if(j==1) goto head;
			else if(j==2) {system("cls");ip_st();goto head;}
			else goto end;
		}
		for(i=p=0;i<num_c;i++)
		{
		...
		if(p!=1)
		{
			...
			scanf("%d",&j);
			if(j==1) goto head;
			else if(j==2) {system("cls");ip_c();goto head;}
			else goto end;
		}
...
a:
	...
	{	
		if(strcmp(ch,st[i].xh)==0) 
		{
			...
			printf("是否删除此条记录？\n是，请按<1>和<Enter>\n否，请按<2>和<Enter>\n请输入：");
			scanf("%d",&j);
			if(j==1) {d_st(i);printf("删除成功!!");system("pause");}
		}
	}
	if(k!=1) 
	{
...
b:
	...
	printf("是否还删除其他的学生信息？\n是：1\n否：0\n请输入：");
	scanf("%d",&j);
	if(j==1) {system("cls");goto head;}
	printf("是否还需删除另外两类的信息？\n课程信息：1\n选课信息：2\n不，没有其他需要删除的了：0\n请输入：");
	scanf("%d",&j);
	if(j==1) {system("cls");del_c();}
	else if(j==2) {system("cls");del_sc();}
}
...
a:
	...
	{	
		if(strcmp(ch,c[i].kh)==0) 
		{
			...
			printf("是否删除此条记录？\n是，请按<1>和<Enter>\n否，请按<2>和<Enter>\n请输入：");
			scanf("%d",&j);
			if(j==1) {d_c(i);printf("删除成功!!");system("pause");}
		}
	}
	if(k!=1) 
	{
...
a:
	...
	for(i=p=0;i<num_sc;i++)
	{
		if(strcmp(xh,sc[i].xh)==0)
		{
			...
			scanf("%d",&j);
			if(j==1) {d_sc(i);printf("删除成功!!");system("pause");}
		}
	}
	if(p!=1)
	{
	...
	printf("是否还删除其他的选课信息？\n是：1\n否：0\n");
	scanf("%d",&j);
	if(j==1) {system("cls");goto a;}
	printf("是否还需删除另外两类的信息？\n学生信息：1\n课程信息：2\n不，没有其他需要删除的了：0\n");
	scanf("%d",&j);
	if(j==1) {system("cls");del_st();}
	else if(j==2) {system("cls");del_c();}
}
...
/*D·选项2---显示*/void _2x()
{
	...
	switch(i)
	{
		case 1 : disp_stud();break;
		case 2 : disp_course();break;
		case 3 : disp_cjlist();break;
		default: cw();
	}
	zc();
}
...
/*D·选项6---导出*/void _6x()
{
	...
	printf("请选择：\n导出学生信息、课程信息和选课信息：1\n导出成绩信息：2\n");
	scanf("%d",&i);
	if(i==1) write_to_file1();
	else if(i==2) write_to_file2();
	else cw();
}
/*D·目录*/void ml()
{
...
again:	
	...
	switch(i)
	{
		...
		case 6: _6x();break;
		case 0: zqend();break;
		default :printf("Sorry! I do not understand your order. Let us check my catalog again. \nPlease choose the one I can understand.");system("pause");goto again;break;
	}
	zz();
}
int main()
